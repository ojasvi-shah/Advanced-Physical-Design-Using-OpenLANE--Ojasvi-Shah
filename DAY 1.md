...
The command when run, sets up a filesystem where the OpenLANE can store the results. This creates a folder inside the **picorv32a** directory which contains the command log files, results, and the reports dumped of the various tool. The folder will be only have the lef files generated by this design setup stage. The cell LEF files *.lef* and technology LEF files *.tlef* merge to generate merged.lef inside **runs/tmp/**
...
## AUTHOR : OJASVI SHAH (NIAS)
## PLEASE DO NOT COPY ANY OF THE AUTHOR'S WORK.
